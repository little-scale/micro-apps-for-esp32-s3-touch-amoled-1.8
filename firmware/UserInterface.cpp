...
bool UserInterface::begin(const DeviceSettings &settings) {
  ...
  Serial.printf("display canvases=%s free_psram=%u\n", canvasesReady ? "ok" : "failed",
                ESP.getFreePsram());
...
void UserInterface::closeWifiSetup() {
  ...
  wifiSetupTouching_ = false;
  // Settings buttons act on touch-down. Do not allow that same physical touch
  // to activate an app control on the next frame.
  touching_ = false;
...
void UserInterface::beginWifiScanAttempt() {
  WiFi.scanDelete();
  // Auto-reconnect can make the ESP32 radio reject a new asynchronous scan.
...
void UserInterface::handleWifiSetupTap(int16_t x, int16_t y) {
  ...
  if (wifiSetupPage_ == WifiSetupPage::Menu) {
    if (inside(x, y, 24, 40, 320, 60)) {
      startWifiScan();
    } else if (inside(x, y, 24, 109, 320, 60)) {
      oscActiveField_ = 0;
      oscInputError_ = false;
      wifiSetupPage_ = WifiSetupPage::Osc;
      drawWifiSetup();
    } else if (inside(x, y, 24, 178, 320, 60)) {
      ...
      drawWifiSetup();
    } else if (inside(x, y, 24, 247, 320, 60)) {
      physicsGravityEdit_ = ballGravity_;
      physicsBouncinessEdit_ = ballBounciness_;
      wifiSetupPage_ = WifiSetupPage::Physics;
      drawWifiSetup();
    } else if (inside(x, y, 24, 342, 320, 64)) {
      closeWifiSetup();
    }
    return;
  }
...
void UserInterface::drawSettingsMenu(Arduino_GFX *target) {
  ...
  drawSetupButton(target, 24, 40, 320, 60, "NETWORK", panelColor_, foregroundColor_,
                  foregroundColor_);
  drawSetupButton(target, 24, 109, 320, 60, "OSC", panelColor_, foregroundColor_,
                  foregroundColor_);
  drawSetupButton(target, 24, 178, 320, 60, "DEVICE", panelColor_, foregroundColor_,
                  foregroundColor_);
  drawSetupButton(target, 24, 247, 320, 60, "PHYSICS", panelColor_, foregroundColor_,
                  foregroundColor_);
  drawSetupButton(target, 24, 342, 320, 64, "EXIT", panelColor_, mutedColor_,
                  foregroundColor_);
}
...
void UserInterface::loop(ControlState &state, const ImuFrame &imu, float micEnergy,
  ...
  readTouch(state);
  // Releasing the Wi-Fi badge opens and draws setup from inside readTouch().
  // Do not let the normal page renderer overwrite any of that full-screen frame.
  if (wifiSetupActive_) return;
  if (controlPage_ == kPageSpectrum && fftCaptureActive_ && spectrumReady) {
    applyFftSpectrum(spectrum, state);
  }
  updatePowerStatus();
  if (controlPage_ == kPageBalls) {
    updateBall(imu);
  } else {
    lastBallUpdateUs_ = 0;
  }
  if (controlPage_ == kPagePendulums) {
    updatePendulum(imu);
  } else {
    for (PendulumState &pendulum : pendulums_) pendulum.lastUpdateUs = 0;
  }
  if (controlPage_ == kPageParticles) {
    updateParticles(micEnergy, imu);
  } else {
  ...
  const bool backgroundChanged = memcmp(previousBackground_, state.background, 3) != 0;
  if (!hasDrawn_ || backgroundChanged) {
    updatePalette(state);
    drawAll(state, wifiConnected, bleEnabled, bleConnected, imuOutputEnabled,
            micOutputEnabled);
    memcpy(previousBackground_, state.background, 3);
    drawnState_ = state;
    hasDrawn_ = true;
    return;
  }

  TouchTarget visibleActive = TouchTarget::None;
  if (touching_) {
    if (controlPage_ == kPageXy && activeTouch_ == TouchTarget::Xy) {
      visibleActive = activeTouch_;
  ...
  if (controlPage_ == kPageXy) {
    valueChanged = different(drawnState_.xyX, state.xyX) ||
                   different(drawnState_.xyY, state.xyY);
  } else if (controlPage_ == kPageFaders) {
    for (uint8_t index = 0; index < 4; ++index) {
      valueChanged |= different(drawnState_.faders[index], state.faders[index]);
    }
  } else if (controlPage_ == kPageButtons) {
    for (uint8_t index = 0; index < 4; ++index) {
      valueChanged |= drawnState_.buttons[index] != state.buttons[index];
    }
  } else if (controlPage_ == kPageBalls) {
    valueChanged = ballDirty_;
    ...
    for (uint8_t key = 0; key < 13; ++key) {
      ...
      valueChanged |= drawnState_.keyboardNotes[note] != state.keyboardNotes[note];
    }
  } else {
    for (uint8_t index = 0; index < kSpectrumBandCount; ++index) {
      valueChanged |= different(drawnState_.spectrum[index], state.spectrum[index]);
    }
  }

  const bool activeChanged = drawnActiveTouch_ != visibleActive;
  const uint32_t drawNowUs = micros();
  const uint32_t drawIntervalUs =
      controlPage_ == kPageParticles ? kParticleDrawIntervalUs : 20000;
  const bool cadenceReady =
      static_cast<uint32_t>(drawNowUs - lastControlDrawUs_) >= drawIntervalUs;
  if (activeChanged ||
      (valueChanged &&
       (controlPage_ == kPageButtons || controlPage_ == kPageKeyboard || cadenceReady))) {
    // Start-to-start pacing includes transfer time, keeping continuous controls at 50 Hz.
    lastControlDrawUs_ = drawNowUs;
    drawCurrentPage(state);
    drawnState_ = state;
  }
  // Status pixels are transferred only when their visible state changes. This avoids a
  // periodic display-bus stall while the particle animation is running.
  const bool topChanged = !topStateValid_ ||
                          ...
                          drawnMicOutputEnabled_ != micOutputEnabled ||
                          drawnBatteryPercent_ != batteryPercent_ ||
                          drawnCharging_ != charging_;
  if (!touching_ && topChanged) {
...
UserInterface::TouchTarget UserInterface::hitTest(int16_t x, int16_t y) const {
  ...
  if (inside(x, y, 90 + kStatusBadgeShiftX, 0, 37, kTopHeight)) {
    return TouchTarget::ImuOutput;
  }
  if (inside(x, y, 127 + kStatusBadgeShiftX, 0, 42, kTopHeight)) {
    return TouchTarget::MicOutput;
  }
...
void UserInterface::readTouch(ControlState &state) {
  ...
  if (pressed) {
    if (!touching_) {
      ...
      touchValueChanged_ = false;
      activeTouch_ = hitTest(x, y);
      wake();
      beginTouch(activeTouch_, x, y, state);
    } else {
      moveTouch(x, y, state);
    }
  } else if (touching_) {
...
void UserInterface::endTouch(ControlState &state) {
  ...
  if (activeTouch_ >= TouchTarget::Button0 && activeTouch_ <= TouchTarget::Button3) {
    ...
    enqueue(event);
  } else if (activeTouch_ == TouchTarget::MicOutput) {
    event.type = UiEventType::ToggleMicOutput;
    enqueue(event);
...
void UserInterface::drawTop(bool wifiConnected, bool bleEnabled, bool bleConnected,
  ...
  // This badge opens all device settings, so use a cog rather than a
  // Wi-Fi-only symbol. Its fill colour still reports Wi-Fi connection state.
  target->fillRect(29 + kStatusBadgeShiftX, 9, 5, 6, wifiGlyph);
  target->fillRect(29 + kStatusBadgeShiftX, 27, 5, 6, wifiGlyph);
  target->fillRect(19 + kStatusBadgeShiftX, 19, 6, 5, wifiGlyph);
  target->fillRect(38 + kStatusBadgeShiftX, 19, 6, 5, wifiGlyph);
  target->fillRect(22 + kStatusBadgeShiftX, 12, 5, 5, wifiGlyph);
  target->fillRect(36 + kStatusBadgeShiftX, 12, 5, 5, wifiGlyph);
  target->fillRect(22 + kStatusBadgeShiftX, 26, 5, 5, wifiGlyph);
  target->fillRect(36 + kStatusBadgeShiftX, 26, 5, 5, wifiGlyph);
  target->fillCircle(31 + kStatusBadgeShiftX, 21, 9, wifiGlyph);
  target->fillCircle(31 + kStatusBadgeShiftX, 21, 3, wifiColor);
  ...
  // The microphone badge gates transport only. Local microphone-driven pages
  // and FFT capture remain active while it is grey.
  const uint16_t micColor = micOutputEnabled ? rgb565(64, 210, 255) : mutedColor_;
  const int16_t micX = 127 + kStatusBadgeShiftX;
  const int16_t micCenterX = micX + 18;
  const uint16_t micGlyph = rgb565(0, 0, 0);
  target->fillRoundRect(micX, 6, 36, 31, 6, micColor);
  target->fillRoundRect(micCenterX - 4, 10, 8, 14, 4, micGlyph);
  target->fillRect(micCenterX - 8, 18, 3, 7, micGlyph);
  target->fillRect(micCenterX + 5, 18, 3, 7, micGlyph);
  target->fillRect(micCenterX - 6, 24, 12, 3, micGlyph);
  target->fillRect(micCenterX - 1, 26, 3, 5, micGlyph);
  target->fillRect(micCenterX - 6, 30, 12, 3, micGlyph);
